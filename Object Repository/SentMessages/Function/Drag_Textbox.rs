<?xml version="1.0" encoding="UTF-8"?>
<WebElementEntity>
   <description></description>
   <name>Drag_Textbox</name>
   <tag></tag>
   <elementGuidId>8b0e0791-8fbb-4417-b23c-bc0e0fbd8a72</elementGuidId>
   <selectorCollection>
      <entry>
         <key>XPATH</key>
         <value>//*/text()[normalize-space(.)='']/parent::*</value>
      </entry>
   </selectorCollection>
   <selectorMethod>XPATH</selectorMethod>
   <useRalativeImagePath>false</useRalativeImagePath>
   <webElementProperties>
      <isSelected>false</isSelected>
      <matchCondition>equals</matchCondition>
      <name>tag</name>
      <type>Main</type>
      <value>html</value>
   </webElementProperties>
   <webElementProperties>
      <isSelected>false</isSelected>
      <matchCondition>equals</matchCondition>
      <name>xmlns</name>
      <type>Main</type>
      <value>http://www.w3.org/1999/xhtml</value>
   </webElementProperties>
   <webElementProperties>
      <isSelected>true</isSelected>
      <matchCondition>equals</matchCondition>
      <name>text</name>
      <type>Main</type>
      <value>
	

    
 ช่องทางแจ้งเรื่องร้องเรียนการทุจริต - University of Phayao : มหาวิทยาลัยพะเยา


   



    
     
     
     
   


   

  
     

         var _gaq = _gaq || [];
         _gaq.push(['_setAccount', 'UA-27269763-1']);
         _gaq.push(['_setDomainName', 'up.ac.th']);
         _gaq.push(['_trackPageview']);

         (function () {
             var ga = document.createElement('script'); ga.type = 'text/javascript'; ga.async = true;
             ga.src = ('https:' == document.location.protocol ? 'https://ssl' : 'http://www') + '.google-analytics.com/ga.js';
             var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(ga, s);
         })();

    
    
(function (w, d, s, l, i) {
w[l] = w[l] || []; w[l].push({
'gtm.start':
new Date().getTime(), event: 'gtm.js'
}); var f = d.getElementsByTagName(s)[0],
j = d.createElement(s), dl = l != 'dataLayer' ? '&amp;l=' + l : ''; j.async = true; j.src =
'https://www.googletagmanager.com/gtm.js?id=' + i + dl; f.parentNode.insertBefore(j, f);
})(window, document, 'script', 'dataLayer', 'GTM-MX5L64D');

	      
   

    
          @media (min-width: 1200px) {
            #page-heading .page-title {
                padding-bottom: 30px;
                
            }
        }
       
        .header-row {
    background: rgb(86, 14, 123);
    color: rgba(255, 255, 255, .75);
}
        .content-padding-xs {
    padding-bottom: 0rem;
  padding-top: 0rem; 
}
        .overlay-pattern {
    z-index: 1;
    position:  absolute;
    top: 0;
    left: 0;
    bottom: 0;
    right: 0;
    padding: 0;
    margin: 0;
    background: url(mp4/bg-overlay-pattern.png);
  
 
}
        .list-services a {
    display: block;
    padding: 2.0rem 0;
}
        #secondary-nav .menu > .menu-item > a {
  
    font-size: 2.2rem;
   
}
       #secondary-nav .sub-menu .menu-item > a {
  
    font-size: 1.8rem;
   
}
       #footer {
    background: rgb(86, 14, 123);
    padding: 5.6rem 0;
    color: #ffffff;
    font-size: 1.2rem;
}
       p, li, span, div {
    font-size: 1.8rem;
}
       #footer a {
    color: #ffffff;
    text-decoration: none;
}
       #footer a:hover {
    color: #e9edea;
}
       #footer .widget-title {
   
    font-size: 1.8rem;
   
    color: rgba(255, 255, 255, .75);
   
}
       #main-footer .widget {

    margin-bottom: 0rem;

}
        @media (min-width: 10px) {
             #logo img {
    
    margin-bottom: 10px;
}
             #page-heading .page-title {
    
    font-size: 1.6rem;
  
}
            
            
            }
        @media (min-width: 480px) {
            #main-nav .menu > .menu-item > a {
   
    font-size: 1.4rem;
   
}

               #logo img {
    
    margin-bottom: 10px;

}
                #page-heading .page-title {
    
    font-size: 1.6rem;
  
}
            }
        
        @media (min-width: 768px) {

            #page-heading .page-title {
                font-size: 1.6rem;
            }

        }
        @media (min-width: 992px) {
 #page-heading .page-title {
                font-size: 3.0rem;
            }

        }
        @media (min-width: 1200px) {
 #page-heading .page-title {
                font-size: 3.0rem;
            }
 
            .logo img {
                width: 7%;
            }
        }

         @media (min-width: 1499px) and (max-width: 1920px) {
        

            .logo img {
                width: 7%;
            }
        }

    
    

    !function (f, b, e, v, n, t, s) {
        if (f.fbq) return; n = f.fbq = function () {
            n.callMethod ?
            n.callMethod.apply(n, arguments) : n.queue.push(arguments)
        };
        if (!f._fbq) f._fbq = n; n.push = n; n.loaded = !0; n.version = '2.0';
        n.queue = []; t = b.createElement(e); t.async = !0;
        t.src = v; s = b.getElementsByTagName(e)[0];
        s.parentNode.insertBefore(t, s)
    }(window, document, 'script',
    'https://connect.facebook.net/en_US/fbevents.js');
    fbq('init', '456587671875742');
    fbq('track', 'PageView');

&lt;img height=&quot;1&quot; width=&quot;1&quot; style=&quot;display:none&quot; src=&quot;https://www.facebook.com/tr?id=456587671875742&amp;ev=PageView&amp;noscript=1&quot;/>

   
    

      


.header-row {
    background: rgba(86, 14, 123, 0.44);
    color: rgba(255, 255, 255, .75);
}
          #header {
          min-height:0px;
          }
             
       
    
        .content-padding-lg {
    padding-bottom: 3.8rem;
    padding-top: 3.8rem;
}
                  .page-order-detail {
    box-shadow: 2px 2px 8px 0 rgba(0,0,0,.2);
    background: #fff;
    padding: 1rem;
    margin-bottom: 1rem;
}
    

#katalon{font-family:monospace;font-size:13px;background-color:rgba(0,0,0,.7);position:fixed;top:0;left:0;right:0;display:block;z-index:999999999;line-height: normal} #katalon div{padding:0;margin:0;color:#fff;} #katalon kbd{display:inline-block;padding:3px 5px;font:13px Consolas,&quot;Liberation Mono&quot;,Menlo,Courier,monospace;line-height:10px;color:#555;vertical-align:middle;background-color:#fcfcfc;border:1px solid #ccc;border-bottom-color:#bbb;border-radius:3px;box-shadow:inset 0 -1px 0 #bbb;font-weight: bold} div#katalon-spy_elementInfoDiv {color: lightblue; padding: 0px 5px 5px} div#katalon-spy_instructionDiv {padding: 5px 5px 2.5px}

    
&lt;iframe src=&quot;https://www.googletagmanager.com/ns.html?id=GTM-MX5L64D&quot;
height=&quot;0&quot; width=&quot;0&quot; style=&quot;display:none;visibility:hidden&quot;>&lt;/iframe>

    
           .notification {
    position: absolute;
    background-color: #fb404b;
    text-align: center;
    border-radius: 10px;
    min-width: 18px;
    padding: 0 5px;
    height: 18px;
    font-size: 14px;
    color: #fff;
    font-weight: 700;
    line-height: 18px;
    top: 17px;
    left: -5px;
}
       #logo {
    padding: 0px 0;
   
}  
         
    
    






	
	

 	
		  	
		  		
					
						
							
								UNIVERSITY OF PHAYAO, THAILAND
								
                                        เว็บไซต์เวอร์ชั่นเดิม 
									
									
								
							
						
					
					
						
							
								
									
									
									
                                            
                                                หน้าหลัก
                                            

                                            
                                                เกี่ยวกับเรา

                                                
                                                  
                                                        ประวัติความเป็นมา
                                                    
                                                       
                                                        สัญลักษณ์ของมหาวิทยาลัย
                                                    
                                                    
                                                        สารจากอธิการบดี
                                                    
                                                    
                                                        โครงสร้างองค์กร
                                                    
                                                   
                                                      
                                                        ผู้บริหารมหาวิทยาลัยพะเยา
                                                        
                                                             
                                                                ประวัติอธิการบดี
                                                            
                                                            
                                                                กรรมการสภามหาวิทยาลัย
                                                            
                                                           
                                                              
                                                                ผู้บริหาร
                                                            
                                                                                                                      
                                                        
                                                    
                                                     
                                                        อำนาจหน้าที่
                                                    
                                                     
                                                        ปรัชญา / ปณิธาน / พันธกิจ / วิสัยทัศน์
                                                    
                                                     
                                                        แผนยุทธศาสตร์
                                                    
                                                    
                                                        แผน/มาตรการต่างๆ
                                                    
                                                    

                                                      
                                                        วัฒนธรรมองค์กร
                                                        
                                                             
                                                                วัฒนธรรมองค์กร
                                                            
                                                            
                                                                ประมวลจริยธรรม และคุณธรรมของบุคลากร
                                                            
                                                           
                                                           
                                                                                                                      
                                                        
                                                    

                                                   
                                                    
                                                     
                                                     
                                                        กฎหมายที่เกี่ยวข้อง
                                                    
                                                 
                                                      
                                                

                                                

                                            
                                          
                                                รางวัล
                                                
                                            

                                         
                                                บุคลากร

                                                
                                                    
                                                   
                                                     
                                                        แนะนำการใช้งาน
                                                    
                                                     
                                                        อีเมล์สำหรับบุคลากร
                                                    

                                                    
                                                        คู่มือและมาตรฐานการปฏิบัติงาน
                                                    
                                                     
                                                     
                                                        คู่มือการให้บริการ
                                                    
                                                    


                                                      
                                                        การให้บริการ
                                                        





                                                            
                                                              
                                                                    ข้อมูลเชิงสถิติการให้บริการ
                                                                       
                                                                   


                                                           
                                                           

                                                              
                                                                รายงานผลสำรวจความพึงพอใจการให้บริการ
                                                            
                                                            
                                                            
                                                                คู่มือด้านผลิตบัณฑิต
                                                            
                                                           
                                                                                                                      
                                                        
                                                    





                                                     
                                                        นโยบายการบริหารทรัพยากรบุคคล
                                                        
                                                              
                                                                นโยบายการบริหารทรัพยากรบุคคล
                                                            
                                                       

                                                              
                                                                    การดำเนินการตามนโยบายการบริหารทรัพยากรบุคคล
                                                                       
                                                                   



                                                            
                                                                    
                                                                    หลักเกณฑ์การบริหารและพัฒนาทรัพยากรบุคคล
                                                                       
                                                                   
                                                          
                                                          
                                                                รายงานผลการบริหารและพัฒนาทรัพยากรบุคคลประจำปี
                                                            
                                                          
                                                                                                                      
                                                        
                                                    
                                                   
                                                   



                                                
                                                     
                                                        คู่มือและมาตรฐาน มคอ.
                                                    
                                                    
                                                        สวัสดิการ
                                                    
                                                    
                                                        การเงิน
                                                    
                                                   
                                                    


                                                  
                                                  
                                                  
                                                   

                                                   
                                                
                                            
                                      
                                          
                                                นิสิตปัจจุบัน
                                                

                                                      
                                                        แนะนำน้อง &quot;นิสิตปัจจุบัน&quot;
                                                    
                                                 
                                                     
                                                        คู่มือนิสิต
                                                    

                                                     
                                                        คู่มือหรือมาตรฐานการให้บริการ
                                                    

                                                     
                                                        อีเมล์สำหรับนิสิต
                                                    
                                                    
                                                        วิชาการ
                                                    
                                                    
                                                        กิจกรรม
                                                    
                                                 
                                                       
                                                        ทุนการศึกษา
                                                        
                                                            
                                                                กองบริการการศึกษา
                                                            
                                                            
                                                                กองกิจการนิสิต
                                                            
                                                             
                                                                                                                      
                                                        
                                                    
                                                    
                                                    

                                                   
                                                
                                            
                                        
                                                คู่มือนิสิต
                                                
                                            

                                          
                                                ศิษย์เก่า
                                                
                                            
                                            
                                                e-Service
                                                
                                            
                                        



                                              
                                                ติดต่อเรา
                                                  
                                                      
                                                          ที่อยู่
                                                      
                                                      
                                                          คำถามที่พบบ่อย
                                                      
                                                      
                                                          เบอร์โทรภายใน
                                                      
                                                         
                                                          ช่องทางแจ้งเรื่องร้องเรียนการทุจริต
                                                      
                                                      
                                                          ข้อเสนอแนะเพื่อการพัฒนา
                                                      
                                                      
                                                          ชมเชย
                                                      
                                                        
                                                          ติดต่อสอบถาม
                                                      
                                                     


                                                    

                                                   
                                                  
                                            





                                                                      
                                        
								
							
						
						
					


					
						
							
							
								
									สมัครเข้าศึกษา 
									
                                    
                                        
											หลักสูตรปริญญา
										
										
											ผู้สนใจเข้าศึกษา
										
                                       
										
                                            หลักสูตรที่เปิดสอน
											
										
                                      
										
											สำหรับครูแนะแนว
										
										 
											หลักสูตรอบรม
										
                                    
									
								
								
									คณะ/หน่วยงาน 
									
								
								
									ข่าว
									
										
											 ข่าวประชาสัมพันธ์
										
										
										   ข่าวประกวดราคา
										
										
										
											ข่าวรับสมัครงาน
										
                                        	
											   UP News
										
                                      
                                        
											    VDO Clips
										
                                        
											    ความเป็นสากล
										
									
								
								
									วิจัย 
									
                                          
											ผลงานตีพิมพ์เผยแพร่
										
                                        	
											ฐานข้อมูลงานวิจัย
										
									
										
											1 คณะ 1 โมเดล
										

                                      
                                        
											โครงการวิจัย
										

                                     
										
								
									
								
                                
									e-Learning
									
								
					        
						
					
					 เมนู
					 บริการ
				
                   		
                 
    
		  			
		  				
		  					
		  						
                                      
 ติดต่อเรา > ช่องทางแจ้งเรื่องร้องเรียนการทุจริต

		  					
		  				
		  			
		  		
   

				
		  	
            
          
        
     
       
		  	
			   

    
     
			  
	  	
	  		
	  			
	  			
	  			
	  					
                      
                      ช่องทางแจ้งเรื่องร้องเรียนการทุจริต
                          ข้อมูลร้องเรียนของท่านจะเก็บเป็นความลับและไม่ถูกเปิดเผย และข้อมูลของท่านจะถูกส่งไปยัง E-mail ของผู้เกี่ยวข้องเท่านั้น เพื่อดำเนินการต่อไป

							  
                                          
                                              
                                                 
                                              
                                              
                                                   
                                              
                                          
                                          
                                              
                                                    
                                              

                                          
                                          
                                              
                                                   
                                       
                                              
                                          
                                           
                                         
                                        
							    
							



	  					
	  				
	  		
	  		
	  	
                            
    


				
			  	
	  			
		
		
		  		
		  			
		  				
		  					ลิงค์เกี่ยวข้อง
		  					
                                      QA Information System
                                       ระบบสนับสนุนการตัดสินใจสำหรับผู้บริหาร
                                        วารสารมหาวิทยาลัยพะเยา
                                       
                                        Speexx
                                        ระบบวาระการประชุมอิเล็กทรอนิกส์
                                        รายวิชาศึกษาทั่วไป
                              ระบบแบบประเมินออนไลน์โดยบัณฑิตและผู้ใช้บัณฑิต
                       

                            

                              


                                	

		  							
		  						
		  				
                              

                              
		  				
		  					
		  						
                                       
		  						
                                   UNIVERSITY OF PHAYAO,  
                                      
                                      . All Rights Reserved.  สำหรับเจ้าหน้าที่ 
                                      
                                      
                                      
                                    
		  					แก้ไขเว็บไซต์ วันที่ 14/11/2019
		  						
		  					
		  				
                                  
		  							
                                
                                       
                                         
                                         
                                        
                                            
		  			
		  				

		  				
		  					ลิงค์อื่นๆ
		  					

                                        สมาคมอธิการบดีแห่งประเทศไทย
                                        กระทรวงการอุดมศึกษา วิทยาศาสตร์ วิจัยและนวัตกรรม 
                                        สำนักงานคณะกรรมการการศึกษาขั้นพื้นฐาน
                                        กลุ่มสถาบันแพทย์ศาสตร์แห่งประเทศไทย
                                        สถาบันทดสอบทางการศึกษาแห่งชาติ
                                        คลังความรู้การใช้ IT โดยกระทรวง ICT
                                     

                                    
                                                
                            
		  				
		  				
		  				
		  				
		  				
		  					
                            
                                  
                                มหาวิทยาลัยพะเยา
                                  19 หมู่ 2 ตำบลแม่กา อำเภอเมืองพะเยา
                                 
                               จังหวัดพะเยา 56000
                                  โทร. 0 5446 6666 
                                      โทรสาร 0 5446 6690
                                      อีเมล์. uppr@up.ac.th
                                      Since 25 มกราคม 2562



                              

                               
                                เว็บไซต์รองรับการใช้งาน
                               
                                      
                                     
                                    

                                   
                                       
                                 
                            
                                
                              
                            

                            
		  				
                          
		  			
		  			
		  		
		  	
		
		   





    
    
        function loadIframe(iframeName, url) {
            var $iframe = $('#' + iframeName);
            if ($iframe.length) {
                $iframe.attr('src', url);
                return false;
            }
            return true;
        }
    

        

  
    
      
        ×Close
        EDIT MODE
      
      
       
      
      
        Close
        
      
    
  


       

		
		
		
		
		
		
		
		
		
	    
	    
	    
        
	 
	    
      
        
     
         $(&quot;#header1&quot;).addClass(&quot;header-content header-content-2&quot;);
         $('#header1').attr('id', 'header');

    
           
       
           $(&quot;#header&quot;).addClass(&quot;header-content header-content-1&quot;);


    


    



id(&quot;footer&quot;)</value>
   </webElementProperties>
   <webElementProperties>
      <isSelected>false</isSelected>
      <matchCondition>equals</matchCondition>
      <name>xpath</name>
      <type>Main</type>
      <value>/html[1]</value>
   </webElementProperties>
   <webElementXpaths>
      <isSelected>true</isSelected>
      <matchCondition>equals</matchCondition>
      <name>xpath:neighbor</name>
      <type>Main</type>
      <value>//*/text()[normalize-space(.)='']/parent::*</value>
   </webElementXpaths>
   <webElementXpaths>
      <isSelected>false</isSelected>
      <matchCondition>equals</matchCondition>
      <name>xpath:position</name>
      <type>Main</type>
      <value>//html</value>
   </webElementXpaths>
</WebElementEntity>
